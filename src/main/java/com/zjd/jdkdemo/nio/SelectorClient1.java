...
//	      SocketChannel sChannel2 = createSocketChannel("mail.csdn.net", 25);
	      SocketChannel sChannel1 = createSocketChannel("localhost", 10001);
	      SocketChannel sChannel2 = createSocketChannel("localhost", 10002);
	      // 注册选择器，侦听所有的事件
	  ...
	  public static void processSelectionKey(SelectionKey selKey) throws IOException {
	    ...
	    if (selKey.isValid() && selKey.isWritable()) {
	      ...
	      if (!s1 && key1.equals(selKey)) {
	        ...
	        buf.clear();
	        buf.put("hi localhost,from c1/n".getBytes());
	        buf.flip();
	        sChannel.write(buf);
	        s1 = true;
	        ...
	        buf.clear();
	        buf.put("hi localhost from c2/n".getBytes());
	        buf.flip();
	        sChannel.write(buf);
	        s2 = true;
	      }
